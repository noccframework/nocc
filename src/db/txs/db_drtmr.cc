...
namespace nocc {
  ...
  namespace db {
...
#if LOCAL_LOCK_USE_RDMA
      DBDrtmr *context_;
      void set_context(DBDrtmr *context) { context_ = context; }
#endif
    ...
    DBDrtmr::RWSet::RWSet(int tid,int cid): tid(tid),cor_id(cid)
    {
      max_length = MAXSIZE;
      elems = 0;
    ...
    inline void DBDrtmr::RWSet::add(RWSetItem &item) {
    ...
    inline bool DBDrtmr::RWSet::lock_all_set() {
    ...
    inline void DBDrtmr::RWSet::release_all_set() {
    ...
    inline bool DBDrtmr::RWSet::check_local_set() {
    ...
    inline int DBDrtmr::RWSet::commit_local_write() {
    ...
    DBDrtmrIterator::DBDrtmrIterator (DBDrtmr *tx,int tableid,bool sec) {
      tx_ = tx;
      if(sec) {
    ...
    bool DBDrtmrIterator::Valid() {
      /* Read will directly read from the latest one */
      return cur_ != NULL && val_ != NULL;
    }

    uint64_t DBDrtmrIterator::Key(){  return iter_->Key();}
    char *DBDrtmrIterator::Value() { return (char *)val_ ;}
    char *DBDrtmrIterator::Node()  { return (char *)cur_; }

    void DBDrtmrIterator::Next() {
      bool r = iter_->Next();
      while(iter_->Valid()) {
        cur_ = iter_->CurNode();
        {
    ...
    void DBDrtmrIterator::Seek(uint64_t key) {
      iter_->Seek(key);
      cur_ = iter_->CurNode();
    ...
    void DBDrtmrIterator::SeekToFirst() { /* TODO ,not implemented. seems not needed */}

    void DBDrtmrIterator::SeekToLast() {/* TODO ,not implemented */}

    // comment ////////////////////////////////////////////////////////////////


    // Real implementation of DrTMR's OCC //////////////////////////////////////
    DBDrtmr::DBDrtmr(rdmaio::RdmaCtrl *cm,RDMA_sched *sched,MemDB *tables,int t_id,Rpc *rpc,int c_id)
       ...
       sched_(sched)
    {
      ...
      rpc_handler_->register_callback(std::bind(&DBDrtmr::lock_rpc_handler,this,_1,_2,_3,_4),RPC_LOCK);
      rpc_handler_->register_callback(std::bind(&DBDrtmr::release_rpc_handler,this,_1,_2,_3,_4),
                                      RPC_RELEASE);
#if ONE_WRITE == 1
      // use one-sided write does not require register the commit RPC handler
#else
      rpc_handler_->register_callback(std::bind(&DBDrtmr::commit_rpc_handler,this,_1,_2,_3,_4),
                                      RPC_COMMIT);
#endif
    ...
    void DBDrtmr::thread_local_init() {
      ...
      if(false == localinit) {
        ...
        random_ = new util::fast_random[ 1 + coroutine_num];
    ...
    void DBDrtmr::_begin(DBLogger *db_logger,TXProfile *p) {
      // init
      abort_ = false;
      thread_local_init();
    ...
    // local gets
    uint64_t DBDrtmr::get(int tableid,uint64_t key,char **val,int len) {
...
#if USE_LOGGER
    inline void prepare_log(int cor_id, DBLogger* db_logger, const DBDrtmr::RWSet::RWSetItem& item){
...
#endif
    void DBDrtmr::write(int tableid,uint64_t key,char *val,int len) {
    ...
    uint64_t DBDrtmr::get_ro(int tableid,uint64_t key,char *val,yield_func_t &yield) {
      char *temp;
    ...
    // local inserts
    void DBDrtmr::insert(int tableid,uint64_t key,char *val,int len) {
    ...
    void DBDrtmr::insert_index(int idx_id,uint64_t key,char *val) {
    ...
    void DBDrtmr::delete_(int tableid,uint64_t key) {
      for(uint i = 0;i < rwset_->elems;++i) {
    ...
    int DBDrtmr::add_to_remote_set(int tableid,uint64_t key,int pid,yield_func_t &yield) {
      int vlen = txdb_->_schemas[tableid].vlen;
      return rrwset_->add(pid,tableid,key,vlen + META_LENGTH,yield);
    }

    int DBDrtmr::add_to_remote_set(int tableid,uint64_t key,int pid) {
      int vlen = txdb_->_schemas[tableid].vlen;
      return rrwset_->add(pid,tableid,key,vlen + META_LENGTH);
    }

    int DBDrtmr::add_to_remote_set_imm(int tableid,uint64_t key,int pid) {
      return add_to_remote_set(tableid,key,pid);
    }

    // get the cached value of remote values
    uint64_t DBDrtmr::get_cached(int idx,char **val) {
    ...
    void  DBDrtmr::get_imm_res(int idx,char **ptr,int size) {
      // direct fetch result from one-sided-remote set
      assert(idx < rrwset_->numItems());
      *ptr = (char *)(rrwset_->kvs_[idx].val + META_LENGTH);
    }

    uint64_t DBDrtmr::get_cached(int tableid,uint64_t key,char **val) {
    ...
    void DBDrtmr::do_remote_reads(yield_func_t &yield) {assert(false);}

    int  DBDrtmr::do_remote_reads() {
      // TODO
      return 0;
    }
