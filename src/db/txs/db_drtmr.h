#ifndef _TX_DRTMR_OCC
#define _TX_DRTMR_OCC
...
namespace nocc {
    ...
    class DBDrtmr : public TXHandler {
    ...
    public:
      DBDrtmr(rdmaio::RdmaCtrl *cm,RDMA_sched *sched,MemDB *tables, int t_id,Rpc *rpc,int c_id = 0); // constructor
      ...
      void get_remote_results(int ){
            // do nothing since one-sided RDMA reads does not
    ...
    private:
      MemDB *txdb_; friend class DBDrtmrIterator; // allow iterator to access private *store*
      bool abort_;
      int thread_id;
      Rpc *rpc_handler_;
      DBLogger *db_logger_;
    ...
    // DBDrtmr uses the same itereator implementation of DBTX.

    /* A transaction wrapper of dbiterator which will not prevent phantom */
    class DBDrtmrIterator : public TXIterator {
    public:
      // Initialize an iterator over the specified list.
      // The returned iterator is not valid.
      explicit DBDrtmrIterator(DBDrtmr *tx, int tableid, bool sec = false);
      ~DBDrtmrIterator() { delete iter_; }
