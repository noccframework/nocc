...
namespace nocc {
	...
	namespace db {
		...
		DBLogger::DBLogger(int thread_id,RdmaCtrl *rdma,View *view,Rpc *rpc_handler)
			: rdma_(rdma), view_(view), thread_id_(thread_id), rpc_handler_(rpc_handler) {
#elif LOGGER_USE_RPC == 2
		DBLogger::DBLogger(int thread_id,RdmaCtrl *rdma,View *view, RDMA_sched* rdma_sched, Rpc *rpc_handler)
			...
			//computer the start location
		  	...
		  	temp_logs_ = new TempLog*[coroutine_num + 1];
...
#endif
#if LOGGER_USE_RPC == 0 || LOGGER_USE_RPC == 2
		...
		int DBLogger::log_setup(uint cor_id){
			...
			for(auto backup = temp_log->mac_backups_.begin(); backup != temp_log->mac_backups_.end();
				...
				// printf("setup remote_log_offset: %lu %lu\n",temp_log->remote_log_offsets_[remote_id],temp_log->remote_ack_offsets_[remote_id]);

				remote_offsets_[remote_id] = (remote_offset + whole_temp_log_size) % THREAD_BUF_SIZE;
				// printf("remote_offset: %lu\n",remote_offsets_[remote_id]);
			}
		...
		int DBLogger::log_end(uint cor_id){
			...
			if(temp_log->mac_backups_.size() != 0){
	            ...
	            *((uint64_t*)(req_buf + sizeof(DBLogger::RequestHeader))) = *((uint64_t*)temp_log->start_);
				...
				rpc_handler_->set_msg((char*) request_header);
				rpc_handler_->send_reqs(RPC_LOGGING_COMMIT,sizeof(uint64_t) + sizeof(DBLogger::RequestHeader),
	                                      temp_log->remote_mac_ids_,temp_log->remote_mac_num_,cor_id);
#else
