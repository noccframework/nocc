    ...
    namespace tpce {
        ...
        BenchWorker(worker_id,true,seed,0,a,b,context)
      {
        ...
        input_generator_ = new CCETxnInputGenerator(*inputFiles,
                                                    totalCustomer,totalCustomer,						       /* 24 hours per day*/
      ...
      uint64_t TpceWorker::get_last_trade_id() {
        ...
        trade_id_ += 1;
        //      fprintf(stdout,"get tid %lu @%d\n",trade_id_ - 1,cor_id_);
        //      assert(decode_trade_mac(res) == current_partition);
      ...
      void TpceWorker::thread_local_init() {
        ...
        tradeResultCache = new TTradeResultTxnInput*[coroutine_num + 1];

        market_feed_process_idx = new int8_t[coroutine_num + 1];
        tickerBuffer = new TTickerEntry[coroutine_num + 1][10];
      ...
      TpceWorker::txn_trade_order(yield_func_t &yield) {
        ...
        //      fprintf(stdout,"%d check trade dts %lu , now %lu\n",cor_id_,v_t.t_dts,now_dts);
#ifdef SANITY_CHECKS
        {
          //	uint64_t header = trade_id >> 32;
          //	int pid = header >> 16; assert(pid == current_partition);
          //	int tid = header & 0x1f;assert(tid == worker_id_);
        }
#endif
...
#if 0
            {
                char *log_buf = next_log_entry(&local_log,16);
                assert(log_buf != NULL);
                sprintf(log_buf,"emplace %lu @%d\n",decode_trade_payload(trade_id),worker_id_);
            }
#endif
//          fprintf(stdout,"emplace %lu @%d\n",decode_trade_payload(trade_id),worker_id_);
        ...
        //      fprintf(stdout,"trade orde done %d\n",res);
        //return txn_result_t(res,acct_bal + hold_assets);
      ...
      txn_result_t TpceWorker::txn_trade_result(yield_func_t &yield) {
        ...
        //
        //      TTradeResultTxnInput input = tradeResultQueue->front();
        //      tradeResultCache[cor_id_] = tradeResultQueue->front();
        //      tradeResultQueue->pop();

        tx_->begin(db_logger_);
        uint64_t trade_dts = CDateTime().GetDate();
        //fprintf(stdout,"trade result process %d %d %d\n",decode_trade_worker(input->trade_id),
        //	      decode_trade_routine(input->trade_id),decode_trade_payload(input->trade_id));
        /* Frame 1 */
        trade::value *tv;
        ...
        if(input.c_id) {
          ...
          while(iter.Valid()) {
            ...
            if(pid == current_partition) {
              ...
              items[secs_count].pid = pid;
              secs_count += 1;
              //	    if(secs_count * sizeof(MWInputItem) + sizeof(MarketWatchInput)
              //	       > MAX_MSG_SIZE - 16 * sizeof(uint64_t)) {
              //	      fprintf(stdout,"case 2 expanse %d %d\n",secs_count,fork_handler.get_server_num());
              //	      assert(false);
              //	    }
            }
            iter.Next();
...
//          set<uint64_t> remote_trades;
          ...
          while(tr_iter.Valid()) {
            ...
            //	  uint64_t seq = tx_->get((char *)(tr_iter.Node()),(char **)(&tr),sizeof(trade_request::value));
            //	  uint64_t seq = 1;
            if(unlikely(seq == 0)) {
              //	  if(1) {
              /* possible, deleted by a concurrent market feed request */
              tr_iter.Next();
              continue;
            }
            ...
            //printSecondTRKey(sec_key);
#if 0
              {
                  char *log_buf = next_log_entry(&local_log,128);
                  uint64_t *key = (uint64_t *)(tr_iter.Key());
                  assert(log_buf != NULL);
                  sprintf(log_buf,
                          "len %d TradeRequest key: Sec %s, status %s, bid %lu, trade id %lu %p\n",tr_len,
                         (char *)(&(key[0])), (char *)(&(key[4])),key[2],key[3],tr_iter.Node());
              }
#endif
