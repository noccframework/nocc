  ...
  namespace oltp {
    ...
    namespace tpcc {
      ...
      txn_result_t TpccWorker::txn_new_order(yield_func_t &yield) {
        ...
        const uint districtID = RandomNumber(rand_generator_, 1, 10);
        //const uint districtID = 1;
        const uint customerID = GetCustomerId(rand_generator_);
        uint64_t c_key = makeCustomerKey(warehouse_id, districtID, customerID);

#define MAX_ITEM 15
        const uint numItems = RandomNumber(rand_generator_, 5, MAX_ITEM);
        ...
        for (uint i = 0; i < numItems; i++) {
          bool conflict = false;
          int item_id = GetItemId(rand_generator_);
          if (NumWarehouses() == 1 ||
                      RandomNumber(rand_generator_, 1, 100) > g_new_order_remote_item_pct) {
            ...
            do {
              supplier_warehouse_id = RandomNumber(rand_generator_, 1, NumWarehouses());
      ...
      txn_result_t TpccWorker::txn_payment(yield_func_t &yield) {
...
#if 1
        //here is the trick
        customerDistrictID = RandomNumber(rand_generator_, 1, NumDistrictsPerWarehouse());
        customerWarehouseID = PickWarehouseId(rand_generator_, warehouse_id_start_, warehouse_id_end_);

        if (likely(NumWarehouses() == 1 ||
                   RandomNumber(rand_generator_, 1, 100) <= 85)) {
          districtID = customerDistrictID;
          warehouse_id = customerWarehouseID;
          d_key = makeDistrictKey(warehouse_id, districtID);
        } else {
          warehouse_id = RandomNumber(rand_generator_,1,NumWarehouses());
          districtID   = RandomNumber(rand_generator_, 1, NumDistrictsPerWarehouse());
          d_key = makeDistrictKey(warehouse_id, districtID);
        ...
        districtID = RandomNumber(rand_generator_,1,NumDistrictsPerWarehouse());
        warehouse_id = PickWarehouseId(rand_generator_,warehouse_id_start_,warehouse_id_end_);

        if(likely(NumWarehouses() == 1 || RandomNumber(rand_generator_,1,100)) <= 85) {
        ...
        d_key = makeDistrictKey(warehouse_id, districtID);
#endif
        const uint64_t addAmount = (uint64_t)(RandomNumber(rand_generator_,100,500000));
        ...
        customer::value *v_c;
#if 1
        if (RandomNumber(rand_generator_, 1, 100) <= 60) {
          // cust by name
          ...
          GetNonUniformCustomerLastNameRun(lastname_buf, rand_generator_);
      ...
      txn_result_t TpccWorker::txn_stock_level(yield_func_t &yield) {
        ...
        const uint warehouse_id = PickWarehouseId(rand_generator_, warehouse_id_start_, warehouse_id_end_);
        const uint threshold = RandomNumber(rand_generator_, 10, 20);
        const uint districtID = RandomNumber(rand_generator_, 1, NumDistrictsPerWarehouse());
      ...
      txn_result_t TpccWorker::txn_super_stock_level(yield_func_t &yield) {
        ...
        //      int num_dists = RandomNumber(rand_generator_, 2, 2 * total_partition);
        ...
        for(uint i = 0;i < num_dists;++i) {
          uint warehouse_id = RandomNumber(rand_generator_,1,NumWarehouses());
          uint districtID = RandomNumber(rand_generator_,1,NumDistrictsPerWarehouse());
          int pid;
      ...
      txn_result_t TpccWorker::txn_micro(yield_func_t &yield) {
        ...
        retry:
          const uint warehouse_id = PickWarehouseId(rand_generator_,1,NumWarehouses());
          const uint districtID = RandomNumber(rand_generator_,1,10);
          uint64_t d_key = makeDistrictKey(warehouse_id, districtID);
